...
public class ProblemDetailsHandler : DelegatingHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (httpResponse.Content.Headers.ContentType?.MediaType == "application/problem+json")
        {
            ...
            try
            {
                problemDetails = await JsonSerializer.DeserializeAsync<ProblemDetails?>(contentStream, jsonSerializerOptions, cancellationToken);
            }
            catch (Exception ex)
            {
            ...
            if (problemDetails == null)
            {
                problemDetails = new ProblemDetails
                {
                    ...
                    Detail = "MediaType was application/problem+json, but for some reason Deserialize returned null"
                };
            }
